...
subjects = {[393 395 407 417 418 423 425 429 437 438 441 443 445 450 455] [396 397 402 406 408 421 427 433 436 446 448 449 456 458] [398 401 403 405 430 431 434]}; % Add in the additional subjects you've tested.
comps = {'P1' 'N1' 'P2'}; 

% Setup main directory to load in Group data from:
in_dir = '/Users/kmccl/Documents/DATA/subjects/Groups/Group GFP/'; % all group .mat files should be here.

% First, calculate Grand Average:
% Load in the group average ERPs:
load([in_dir,'GFP_hla_sl_erps.mat']); % 'hla_spl_erps', 'msecs'
load([in_dir,'GFP_hla_spl_erps.mat']); % 'hla_sl_erps'
load([in_dir,'GFP_hlu_sl_erps.mat']);% 'hlu_spl_erps'
load([in_dir,'GFP_hlu_spl_erps.mat']); % 'hlu_sl_erps'
load([in_dir,'GFP_nh_sl_erps.mat']); % 'nh_spl_erps'
load([in_dir,'GFP_nh_spl_erps.mat']); % 'nh_sl_erps'

% Now, average across the groups, at least for now separately for the SPL and SL
% conditions...
GFP_grand_avg_spl = (GFP_hla_spl_erps + GFP_hlu_spl_erps + GFP_nh_spl_erps)/3;
GFP_grand_avg_sl = (GFP_hla_sl_erps + GFP_hlu_sl_erps + GFP_nh_sl_erps)/3;
...
figure,plot(msecs, GFP_grand_avg_spl);
...
for idx = 1:length(x)
    ...
    amp = GFP_grand_avg_spl(latidx);
    
    lats(idx) = lat;
    amps(idx) = amp;
end % for idx
% Write out these Latency and Amplitude values to a text file:
fid = fopen([in_dir,'GFP_GrandMeanValues_SPL.txt'],'wt');
for x = 1:length(lats)
    fprintf(fid,'%g\t %g\n',lats(x), amps(x));
end % for x
fclose(fid);
...
%     figure
%     subplot(1,3,1)
%     topoplot(nh_spl_erps(:,idx(x)),EEG.chanlocs,'maplimits',[-5 5],'style','map');
%     title(sprintf('NH SPL, Time: %s msec',num2str(times_to_plot(x))))
%     
%     subplot(1,3,2)
%     topoplot(hlu_spl_erps(:,idx(x)),EEG.chanlocs,'maplimits',[-5 5],'style','map');
%     title(sprintf('HLU SPL, Time: %s msec',num2str(times_to_plot(x))))
%     
%     subplot(1,3,3)
%     topoplot(hla_spl_erps(:,idx(x)),EEG.chanlocs,'maplimits',[-5 5],'style','map');
...
figure,plot(msecs, GFP_grand_avg_sl);
[x, y] = ginput(numel(comps)); % Select the peak P1, N1, and P2 in that order.
% x = the Latencies clicked
% y = the Amplitudes at those Latency.
close;
...
for idx = 1:length(x)
    ...
    % now find the amplitude at that latency.
    amp = GFP_grand_avg_sl(latidx);
    
    lats(idx) = lat;
    amps(idx) = amp;
end % for idx
% Write out these Latency and Amplitude values to a text file:
fid = fopen([in_dir,'GFP_GrandMeanValues_SL.txt'],'wt');
for x = 1:length(lats)
    fprintf(fid,'%g\t %g\n',lats(x), amps(x));
end % for x
fclose(fid);
...
%     figure
%     subplot(1,3,1)
%     topoplot(nh_sl_erps(:,idx(x)),EEG.chanlocs,'maplimits',[-5 5],'style','map');
%     title(sprintf('NH SL, Time: %s msec',num2str(times_to_plot(x))))
%     
%     subplot(1,3,2)
%     topoplot(hlu_sl_erps(:,idx(x)),EEG.chanlocs,'maplimits',[-5 5],'style','map');
%     title(sprintf('HLU SL, Time: %s msec',num2str(times_to_plot(x))))
%     
%     subplot(1,3,3)
%     topoplot(hla_sl_erps(:,idx(x)),EEG.chanlocs,'maplimits',[-5 5],'style','map');
...
%     ch_to_plot=(PCA_comp_chan )
% ch_to_plot= 61
window_length = 28; % ms, Will plot +/- 20/40/80 ms on each side of the peak latency.
window_samples = round(window_length/(1000/EEG.srate));
fid = fopen([in_dir,'GFP_ALL_Latencies.txt'],'wt');
fid2 = fopen([in_dir,'GFP_ALL_Amplitudes.txt'],'wt');
for c = 1:numel(conds) % loop through each condition.
    % Load in the text file with the Grand Average Peak Latencies for
    % this condition:
    fid3 = fopen([in_dir,'GFP_GrandMeanValues_',conds{c},'.txt']);
    ...
    for g = 1:numel(groups) % loop through each group    
        ...
        for s = 1:length(subjects{g}) % loop through each subject
            ...
            load([group_dir,conds{c},num2str(subjects{g}(s)),'_GFP.mat']);
...
% end
            ...
            %for co = 1:numel(comps)
                ...
                [P1_amp,P1_idx]=max(GFP_filt_erps([latidx-window_samples:latidx+window_samples])');
                P1_window=msecs(latidx-window_samples:latidx+window_samples),GFP_filt_erps([latidx-window_samples:latidx+window_samples])';
                ...
                [N1_amp,N1_idx]=max(GFP_filt_erps([latidx-window_samples:latidx+window_samples]));
                 N1_window=msecs(latidx-window_samples:latidx+window_samples),GFP_filt_erps([latidx-window_samples:latidx+window_samples])';
                ...
                [P2_amp,P2_idx]=max(GFP_filt_erps([latidx-window_samples:latidx+window_samples])');
                 P2_window=msecs(latidx-window_samples:latidx+window_samples),GFP_filt_erps([latidx-window_samples:latidx+window_samples])';
